...
close_list = set()  # Use a set for faster membership checking

def movegen(current_node):
    global active_node

    close_list.add(current_node)  # Add the current node to the set of explored nodes
    ...
    if current_node in tree:
        ...
        active_node = open_stack[0]  # Set active node as the first node in the open stack
